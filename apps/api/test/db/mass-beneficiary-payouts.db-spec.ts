...
dbDescribe('Mass beneficiary payouts :: calculate_mass_payout_totals_and_balance', () => {
  it('returns totals and sufficient balance flag', async () => {
    await withRollback(async (client) => {
      const ctx = await seedMassPayoutCtx(client);
      const payoutData = [
        { amount: 5000, recipient_name: 'Alice', recipient_phone: '+22507000001' },
        { amount: 3000, recipient_name: 'Bob', recipient_phone: '+22507000002' },
      ];
...
dbDescribe('Mass beneficiary payouts :: create_mass_beneficiary_payout', () => {
  it('inserts multiple beneficiary_payouts rows atomically', async () => {
    await withRollback(async (client) => {
      const ctx = await seedMassPayoutCtx(client);
      const payoutData = [
        { amount: 4000, recipient_name: 'One', recipient_phone: '+22507000011' },
        { amount: 3500, recipient_name: 'Two', recipient_phone: '+22507000012' },
      ];

      const rows = await callFn(client, 'public.create_mass_beneficiary_payout', {
        p_merchant_id: ctx.merchantId,
        p_provider_code: 'WAVE',
        p_currency_code: 'XOF',
        p_payout_data: payoutData,
...
dbDescribe('Mass beneficiary payouts :: create_mass_beneficiary_payout_with_wave', () => {
