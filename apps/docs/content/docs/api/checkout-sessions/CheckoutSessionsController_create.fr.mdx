...
| `require_email` | Non | `boolean` | Si `true`, affiche et exige l’e-mail. Si `false`, masque l’e-mail (les règles de contact peuvent imposer le téléphone). Défaut si omis : `true`. |
| `require_phone` | Non | `boolean` | Si `true`, affiche et exige le téléphone. Si `false`, téléphone optionnel. Défaut si omis : `false`. |
