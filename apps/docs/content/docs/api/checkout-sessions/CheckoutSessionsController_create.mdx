...
| `require_email` | No | `boolean` | When `true`, show and require email. When `false`, hide email (contact rules may still require phone). Default when unset: `true`. |
| `require_phone` | No | `boolean` | When `true`, show and require phone. When `false`, show phone as optional. Default when unset: `false`. |
