...
| `` `error.code` ``| `` `string` ``                   | Machine-readable code (e.g. `unauthorized`, `idempotency_key_required`, `validation_failed`).                                              |
...
- `"code": "rate_limit_exceeded"`. Response includes `Retry-After`.

Stable codes also include `validation_failed` (field-level `details`), `amount_invalid`, `invalid_cursor`, and `api_access_suspended`.
