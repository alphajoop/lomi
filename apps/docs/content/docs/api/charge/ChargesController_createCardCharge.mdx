...
> **Not available yet.** Card direct charges are turned off while the card rails are being finalized; this endpoint currently returns `503 service_unavailable`. Use [hosted checkout](/build/checkout) for card payments today.
