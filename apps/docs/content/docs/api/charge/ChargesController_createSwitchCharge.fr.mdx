...
> **Pas encore disponible.** Les charges Switch (carte côté serveur) sont désactivées le temps de finaliser les rails carte ; cet endpoint renvoie actuellement `503 service_unavailable`. Utilisez le [checkout hébergé](/build/checkout) pour les paiements carte.
