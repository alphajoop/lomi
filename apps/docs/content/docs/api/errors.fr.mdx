...
- `"code": "idempotency_key_reused"` : la clé a été réutilisée avec un autre corps.
- `"code": "idempotency_in_progress"` : une requête avec cette clé est encore en cours.

**`` `429` `` Too Many Requests:**

- `"code": "rate_limit_exceeded"`. La réponse inclut `Retry-After`.

Autres codes stables : `validation_failed` (détails par champ), `amount_invalid`, `invalid_cursor`, `api_access_suspended`.
