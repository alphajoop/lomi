...
Refunds a completed transaction (card, Wave, or MTN). Merchant balance updates immediately. Supports full and partial refunds. In test mode, MTN refunds are ledger-only (no MTN API call). In live mode, MTN requires a RequestToPay reference on the original transaction.
...
| --- | --- | --- | --- |
| `transaction_id` | Oui | `string` | UUID of a completed transaction (card, Wave, or MTN) |
