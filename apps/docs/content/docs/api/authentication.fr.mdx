...
Sans `` `Lomi-Account` ``, les requêtes se comportent comme des appels API marchands classiques. Avec cet en-tête, la clé API identifie l’opérateur et le compte `` `acct_...` `` est le compte membre ciblé. Voir **[lomi. Network](/resources/network)** pour les endpoints pris en charge et les capacités requises.
