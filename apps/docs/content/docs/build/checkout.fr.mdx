...
<Callout type="info">
  **À savoir :** les sessions expirent par défaut après **60 minutes**. Les clés test s’exécutent en [mode sandbox](/start/sandbox-payments). Règles transverses (codes promo, lignes, état du paiement) : **[Comportement du tunnel](/build/payments/checkout-behavior)**.
</Callout>
...
| `require_name` / `require_email` / `require_phone` / `require_billing_address` | Optionnel, contrôle les champs du formulaire ; voir [Champs du formulaire checkout](/build/payments/checkout-behavior#checkout-form-fields) |
...
- **Fournir `amount`**: doit correspondre au prix récurrent catalogue pour la validation (sauf prorata au moment du paiement ; voir [Comportement du tunnel: Abonnements](/build/payments/checkout-behavior#subscription-checkout)).
...
Voir [Widget de paiement intégré](/build/embed-widget) pour modal, inline et self-hosted.
...
Les **webhooks sortants marchands** (lomi. appelle **votre** URL) utilisent l’énumération **`webhook_event`** documentée dans [Webhooks](/build/webhooks). Le champ HTTP `` `event` `` et l’en-tête `` `X-Lomi-Event` `` reprennent ces valeurs (par ex. `` `PAYMENT_SUCCEEDED` ``).
