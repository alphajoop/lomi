...
Les détails de l’URL et des exemples de corps figurent dans [Clients](/api/customers/CustomersController_createPortalLaunchSession).
...
## Orienter les clients vers le portail

Trois façons d’y accéder :

### 1. URL organisation par défaut (connexion en libre-service)

```
https://customers.lomi.africa/o/{votre-slug-org}
```

Les clients saisissent l’e-mail ou le téléphone utilisé au checkout. Aucun appel API marchand requis. Copiez cette URL depuis **Paramètres → Checkout → Storefront → Portail client**.

### 2. Lancement pré-authentifié (application marchande)

Lorsque le client est déjà identifié dans votre app, créez une session de lancement côté serveur et redirigez vers `launch_url`. Si la session inclut un `customer_id`, le portail **ignore le second OTP** (lancement de confiance).
...
Les reçus client incluent l’URL portail de votre organisation (`customers.lomi.africa/o/{slug}`) pour que les acheteurs reviennent depuis leur boîte mail.

## API headless (optionnel)

Pour des UI portail sur mesure, utilisez le jeton bearer de session portail issu d’un lancement de confiance ou d’un flux OTP :
...
Lorsque `allow_payment_method_update` est activé dans la politique portail (défaut : oui), les clients peuvent :
...
3. Persister la carte avec votre flux d’attache (le portail hébergé appelle la edge function attach après confirmation)

Utilisez `POST /customer-portal/payment-methods/{id}/default` pour définir la carte par défaut et synchroniser les abonnements actifs. Utilisez `DELETE /customer-portal/payment-methods/{id}` pour retirer une carte (bloqué si c’est la seule carte d’un abonnement facturable).

### Reprise après paiement échoué

Pour les abonnements carte `past_due`, `POST /customer-portal/subscriptions/{id}/retry-payment` tente un débit hors session avec la carte enregistrée par défaut. En cas de succès, le renouvellement est enregistré et le dunning est levé. En cas d’échec, la réponse peut inclure un repli `checkout_url` (même chemin que le dunning de renouvellement automatisé).
...
- **Mise à jour des moyens de paiement** : permettre aux clients d’ajouter, retirer et définir une carte par défaut
