...
Generate a one-time hosted customer portal URL for a customer in your organization.

- Endpoint: `POST /customers/{id}/portal-launch-session`
- Intended caller: merchant backend (API key auth)
- Output: `launch_url` + `launch_token` + flow metadata

Use this when you need a hosted handoff from your product to the customer portal.
...
See full flow and production guidance in [Customer portal](/build/customer-portal).
