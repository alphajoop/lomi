...
<Callout type="info">
  Network requests use an active Operator secret API key plus `Lomi-Account: acct_...`. Without `Lomi-Account`, every endpoint behaves like the normal merchant API. Network is invite-only: your organization must be approved as an **Operator** before onboarding applies.
</Callout>

## Onboarding

Every step uses endpoints already supported with the `Lomi-Account` header. There are no extra Network-only routes.
...
If the membership is missing or inactive, the API returns `network_membership_not_found` or `network_membership_not_active`. See [Common authorization errors](#common-authorization-errors).
...
See [Go live](/start/go-live) and [Verify payments](/build/reliability/verify-payments).
