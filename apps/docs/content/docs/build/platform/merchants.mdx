...
Requests require authentication using your API key in the `` `X-API-Key` `` header. See the [Authentication](/start/api-keys) guide.

For organization-level settings, pricing, and metrics, prefer the [Organizations](/build/platform/organizations) API when your integration key is org-scoped.
...
_(See [Data Models](/api/data-models#merchant-object) for property descriptions)_
...
- See the [Errors](/api/errors) guide for general error handling information.
