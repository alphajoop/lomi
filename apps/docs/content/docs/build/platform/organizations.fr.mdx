...
Pour la plupart des intégrations, l’**organisation** est la portée principale des clés API, soldes et paramètres. L’API [Marchands](/build/platform/merchants) reste utile pour les profils marchands et les chemins qui exposent encore un identifiant marchand.
...
|-------|------|-------------|
| `mrr` | `number` | Revenu récurrent mensuel (abonnements actifs, devise par défaut de l’org) |
| `arr` | `number` | Revenu récurrent annuel (`mrr` × 12) |
| `total_revenue` | `number` | Volume total des paiements complétés, converti dans la devise par défaut de l’org |
...
Ces champs correspondent à la ressource **organization** renvoyée par `GET /organizations` (voir [référence API](/api)). Les **modes** tarifaires (`volume_tiered`, `fixed`, `custom`) sont décrits ci-dessus et sous [Tarification](/resources/merchant-of-record/pricing) ; ils n’apparaissent pas forcément comme colonne dédiée dans chaque réponse.
...
| `mrr` | `number` | MRR (abonnements actifs, devise par défaut de l’org) |
| `arr` | `number` | ARR (`mrr` × 12) |
| `merchant_lifetime_value` | `number` | LTV prédictive par client (valeur client x durée de vie moyenne) |
