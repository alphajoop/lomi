...
Génère une URL portail client hébergée à usage unique pour un client de votre organisation.

- Endpoint : `POST /customers/{id}/portal-launch-session`
- Appelant prévu : backend marchand (auth clé API)
- Sortie : `launch_url` + `launch_token` + métadonnées de flux

Utilisez ceci pour une passation hébergée depuis votre produit vers le portail client.
...
Voir le flux complet et les conseils production dans [Portail client](/build/customer-portal).
