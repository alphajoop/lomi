...
<Callout type="info">
  Les SDK exposent des helpers **`/subscriptions`** pour lister, récupérer, mettre à jour et résilier des abonnements. Exemples par route et transitions de statut : **[Abonnements](/api/subscriptions)**. Les champs immuables et les règles d’annulation suivent les descriptions OpenAPI de chaque opération.
</Callout>

<Callout type="info">
  Pour les parcours d’inscription (essais, types de premier paiement, montants de session), voir **[Comportement du tunnel : Abonnements](/build/accept/checkout-behavior#subscription-checkout)**.
</Callout>
...
- [Checkout hébergé](/build/accept/checkout) ou [liens de paiement](/build/accept/payment-links) avec un `product_id` récurrent
...
Surveillez les **événements webhook** pour suivre les confirmations de paiement à chaque cycle. Écoutez `SUBSCRIPTION_RENEWED` et les `PAYMENT_FAILED` liés au renouvellement ; voir [Comportement du tunnel : Abonnements](/build/accept/checkout-behavior#subscription-checkout).
...
2. **Écouter `PAYMENT_FAILED`** sur les transactions de renouvellement : le client peut devoir payer via le lien de renouvellement ou mettre à jour une carte dans le [portail client](/build/billing/customer-portal).
...
Pour les produits **à l’usage / facturation à la consommation** (appels API, crédits, sièges), voir **[Facturation à l’usage](/build/billing/usage-billing)**: les abonnements metered sont distincts des abonnements récurrents checkout.

## Lister les abonnements

**Référence API :** [Lister les abonnements](/api/subscriptions/SubscriptionsController_findAll), `page` et `pageSize`.
...
**Référence API :** [Abonnements par client](/api/subscriptions/CustomersController_getSubscriptions)
...
**Référence API :** [Obtenir un abonnement](/api/subscriptions/SubscriptionsController_findOne)
...
**Référence API :** [Annuler un abonnement](/api/subscriptions/SubscriptionsController_cancel)
...
**Référence API :** [Annuler la résiliation](/api/subscriptions/SubscriptionsController_resume)
...
**Référence API :** [Changer de plan](/api/subscriptions/SubscriptionsController_changePlan)
...
Abonnez-vous à ces événements en **SCREAMING_SNAKE_CASE** (comme sur [Webhooks](/build/reliability)) :
...
- [Lister les abonnements](/api/subscriptions/SubscriptionsController_findAll)
- [Obtenir un abonnement](/api/subscriptions/SubscriptionsController_findOne)
- [Abonnements par client](/api/subscriptions/CustomersController_getSubscriptions)
- [Annuler un abonnement](/api/subscriptions/SubscriptionsController_cancel)
- [Annuler la résiliation](/api/subscriptions/SubscriptionsController_resume)
- [Changer de plan](/api/subscriptions/SubscriptionsController_changePlan)
- [Mettre à jour un abonnement](/api/subscriptions/SubscriptionsController_update)
- [Comportement du tunnel : abonnements](/build/accept/checkout-behavior#subscription-checkout)
