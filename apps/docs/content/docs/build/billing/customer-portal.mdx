...
<DocsScreenshot name="build/customer-portal" alt="lomi. customer portal for payment history and subscriptions" />
...
See endpoint details and payload examples in [Customers](/api/customers/CustomersController_createPortalSession).
...
1. Create a card setup intent via `POST /customer-portal/payment-methods/setup-intent`
2. Confirm the setup intent client-side with your card payment SDK
3. Persist the card with your attach flow (hosted portal calls the attach edge function after confirmation)
