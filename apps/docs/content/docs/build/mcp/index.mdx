...
Your **`LOMI_SECRET_KEY`** is all you need: it both unlocks lomi.’s hosted MCP endpoint and scopes every tool call to your organization.
...
OAuth-capable MCP clients (VS Code, Claude Desktop, and others) can connect without pasting a secret key:

1. Add the hosted MCP server **without** an `x-lomi-api-key` header:
...
2. When the client connects, it opens a browser window to **Connect with lomi.**
3. Sign in to the dashboard, choose your organization, and approve access (read-only by default).
4. The client receives a scoped token and can call MCP tools immediately.
...
1. Discover the resource server at `GET https://mcp.lomi.africa/.well-known/oauth-protected-resource`
