...
`Idempotency-Key` is **required** on money-moving writes: `POST /checkout-sessions`, `POST /payment-requests`, `POST /refunds`, `POST /payouts`, and `POST /charge/*`. Missing keys return `400` `idempotency_key_required`.
