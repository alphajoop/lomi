...
When the response is served from the idempotency cache, the API adds **`Idempotency-Cache-Hit: true`**. Supported today on **`POST /checkout-sessions`** and **`POST /payment-requests`**.
