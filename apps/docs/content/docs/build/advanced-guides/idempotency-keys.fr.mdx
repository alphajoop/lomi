...
`Idempotency-Key` est **obligatoire** sur les écritures financières : `POST /checkout-sessions`, `POST /payment-requests`, `POST /refunds`, `POST /payouts` et `POST /charge/*`. Une clé manquante renvoie `400` `idempotency_key_required`. Une réponse rejouée ajoute `Idempotency-Cache-Hit: true`. Une même clé avec un autre corps renvoie `409` `idempotency_key_reused`. Un doublon en cours renvoie `409` `idempotency_in_progress` (réessayez avec la même clé).
