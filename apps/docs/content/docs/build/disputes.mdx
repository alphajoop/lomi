...
Disputes are created automatically when Stripe sends `charge.dispute.*` events for **card** payments. Use the read API to list and inspect disputes for your organization, subscribe to merchant webhooks, or open **Settings → Support → Disputes** in the [dashboard](https://dashboard.lomi.africa).

<Callout type="info">
  v1 covers **read access** and merchant webhooks (`DISPUTE_CREATED`, `DISPUTE_UPDATED`, `DISPUTE_CLOSED`). Evidence submission and MoMo disputes are not supported yet. Submit dispute evidence in the [Stripe Dashboard](https://dashboard.stripe.com/disputes) until an in-product flow ships.
</Callout>
...
<Callout type="warn">
  lomi. stores a simplified status (`pending`, `resolved`, `closed`). Stripe outcomes such as `won` or `lost` are reflected in `resolution_details` while status is often `resolved`. `DISPUTE_CLOSED` webhooks can fire when Stripe closes a dispute even if the stored status is `resolved`, not `closed`.
</Callout>
...
| `reason` | Stripe dispute reason (e.g. `fraudulent`, `duplicate`) |
| `status` | `pending`, `resolved`, or `closed` |
| `stripe_dispute_id`, `stripe_charge_id` | Stripe references |
...
3. Treat events as idempotent (Stripe may retry; lomi. deduplicates dispute creation).

| Event | When it fires |
| --- | --- |
| `DISPUTE_CREATED` | Cardholder opened a dispute |
| `DISPUTE_UPDATED` | Status or evidence changed on Stripe |
| `DISPUTE_CLOSED` | Stripe sent `charge.dispute.closed` |
...
- [Cards](/build/cards): card payments that can receive disputes
