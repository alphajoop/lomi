...
WooCommerce, Magento 2, and PrestaShop talk only to lomi.’s **public REST API** (`/checkout-sessions`). They do **not** call Supabase RPC or internal-only Nest endpoints.
...
| Sell from a hosted product catalog (no store plugin) | **[Storefront](https://store.lomi.africa)** with [Products](/build/products) (`display_on_storefront`) |
