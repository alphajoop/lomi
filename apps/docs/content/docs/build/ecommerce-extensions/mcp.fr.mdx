...
## Package npm (`@lomi./mcp`)

```bash
npx -y @lomi./mcp
```

Les modèles de configuration stdio/HTTP sont dans `apps/mcp/examples/`. Définissez **`LOMI_API_KEY`** (ou **`X_API_KEY`**) et **`LOMI_API_BASE_URL`** (`https://api.lomi.africa` ou `https://sandbox.api.lomi.africa`).

## Recherche d’outils, ressources et prompts

- **`lomi_search_tools`** — recherche par mot-clé dans le catalogue généré.
...
Dans **Développeurs → Clés API**, le panneau **Connecter MCP** fournit des extraits à copier. Définissez **`VITE_LOMI_MCP_URL`** pour préremplir l’URL MCP hébergée.
