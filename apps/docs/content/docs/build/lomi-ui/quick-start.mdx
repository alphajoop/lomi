...
Orange Money is no longer a default provider id. BNPL uses `bnpl`. Several checkout components now expect hosted-checkout props such as `organizationName`, `fullName`, and `payButtonBgColor` instead of the older simplified examples.
