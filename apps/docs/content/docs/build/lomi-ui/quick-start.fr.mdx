...
Le BNPL utilise `bnpl`. Plusieurs composants checkout attendent désormais des props du checkout hébergé comme `organizationName`, `fullName` et `payButtonBgColor` au lieu des anciens exemples simplifiés.
