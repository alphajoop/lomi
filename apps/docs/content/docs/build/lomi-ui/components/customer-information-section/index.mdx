...
description: Hosted checkout personal information form stack.
---
...
| `isPhoneValid` | `boolean` | When false, shows the phone validation error. |
...
| `onCountryChange` | `(country: string) => void` | Country select handler. |
| `onPhoneValidationChange` | `(isValid?: boolean) => void` | Phone validation callback. |
