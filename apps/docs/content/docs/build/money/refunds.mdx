...
Use **`POST /refunds`** to refund a **completed** transaction. Supported providers are **card**, **Wave**, and **MTN** (same flows as the dashboard). Your **balance is updated immediately**; customer credit timing depends on the payment type.

<TaskSurfaces task="refunds-and-disputes" />

<Callout type="info">
  **MTN (live):** the original payment must have a provider reference (`provider_checkout_id` from RequestToPay). Refunds use the MTN Disbursement refund API and are asynchronous on MTN's side; lomi. polls until completion. **Test mode** refunds are ledger-only (no MTN API call).
</Callout>
