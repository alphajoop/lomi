...
description: 'Les webhooks permettent à votre application de recevoir des notifications en temps réel sur les événements de votre compte lomi. — paiements réussis, nouveaux abonnements, transactions échouées, etc.'
---

import { Callout } from 'fumadocs-ui/components/callout';

Au lieu d’interroger constamment l’API pour détecter les changements, vous pouvez configurer lomi. pour qu’elle envoie des requêtes HTTP POST vers une URL que vous contrôlez dès qu’un événement précis se produit.
...
- `SUBSCRIPTION_UPDATED` : un abonnement a changé (pause, reprise, changement de plan, annulation planifiée, etc.). Le corps inclut `previous_attributes`.
- `SUBSCRIPTION_RENEWED` : un abonnement a été renouvelé avec succès.
- `SUBSCRIPTION_CANCELLED` : un abonnement a été annulé ou a expiré.

## 1. Mettre en place votre URL webhook

Avant de configurer un webhook dans lomi., préparez une URL sur votre serveur qui :
...
3.  **Préparer le HMAC :** avec votre `Signing Secret` conservé en sécurité (la chaîne `whsec_…` fournie par lomi. pour cette URL) comme clé, calculez le hachage HMAC SHA256 de la chaîne du corps brut.
...
Remplacez `'your-webhook-signing-secret'` par le secret réel de votre URL (de préférence récupéré via des variables d’environnement comme `process.env.LOMI_WEBHOOK_SECRET`).
...
Votre URL DOIT répondre rapidement à la requête webhook avec un code HTTP dans la plage `2xx` (par ex. `200 OK`). Cela accuse réception auprès de lomi..
...
Vous pouvez tester votre intégration avec la fonction « Test Webhook » du tableau de bord lomi. (**Developers → Webhooks → sélectionner l’URL → Test**). Cela envoie un événement de test `PAYMENT_SUCCEEDED` vers votre URL configurée, ce qui permet de vérifier la réception et la vérification de signature sans transaction réelle.
...
- **Utiliser HTTPS :** assurez-vous que votre URL utilise HTTPS.
...
- **Consulter les journaux de livraison :** le tableau de bord lomi. (**Developers → Webhooks → sélectionner l’URL → Logs**) affiche l’historique des tentatives de livraison, les codes succès/échec et les corps de réponse capturés par lomi.. Utile pour diagnostiquer les problèmes.
