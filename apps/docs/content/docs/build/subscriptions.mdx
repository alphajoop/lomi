...
<DocsScreenshot name="build/subscriptions" alt="Subscription detail in the lomi. dashboard" />
...
lomi. bills recurring subscriptions automatically when the product and checkout flow created the instance. Your integration should:

1. **Listen for `SUBSCRIPTION_RENEWED`** — successful billing cycle; extend access or send a receipt.
2. **Listen for `PAYMENT_FAILED`** on renewal transactions — customer may need to update payment method in the [customer portal](/build/customer-portal) or a new checkout session.
