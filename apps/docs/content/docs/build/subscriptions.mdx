...
When signup saved a Stripe payment method (`provider_customer_id` + `provider_payment_method_id`), renewals run **off-session** on `next_billing_date`. A daily job charges the saved card and records the renewal transaction.

- Webhook: **`SUBSCRIPTION_RENEWED`** on success
- Webhook: **`PAYMENT_FAILED`** on renewal failure (with Stripe retry/dunning where configured)

### Wave / MTN subscriptions (manual renewal checkout)

Mobile money does not expose a saved off-session token like cards. When no Stripe payment method exists, lomi. **does not auto-debit** the wallet. Instead:
