...
- [Hosted checkout](/build/checkout) or [payment links](/build/payment-links) with a recurring `product_id`
...
Monitor **webhook events** to track payment confirmations for each billing cycle. Listen for `SUBSCRIPTION_RENEWED` and renewal-related `PAYMENT_FAILED` events, see [Checkout behavior: Subscription checkout](/build/payments/checkout-behavior#subscription-checkout).
...
2. **Listen for `PAYMENT_FAILED`** on renewal transactions, customer may need to pay the renewal checkout link or update a card in the [customer portal](/build/customer-portal).
...
For **metered / usage-based** products (API calls, credits, seats), see **[Usage billing](/build/usage-billing)**-usage subscriptions and meters are separate from recurring checkout subscriptions.
...
4. For failed renewals, use the [customer portal retry payment](/build/customer-portal#failed-payment-recovery) headless endpoint or redirect customers to the hosted portal billing/subscriptions pages.
...
- [Checkout behavior: subscriptions](/build/payments/checkout-behavior#subscription-checkout)
