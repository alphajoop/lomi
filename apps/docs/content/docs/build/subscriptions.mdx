...
3. **Poll or list subscriptions** via `GET /subscriptions` or `GET /customer-subscriptions` when webhooks are delayed, use `status` and `next_billing_date` from the API response only.
