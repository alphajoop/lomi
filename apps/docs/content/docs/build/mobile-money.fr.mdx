...
description: 'Utilisez le checkout hébergé, les liens de paiement ou des flux directs pour accepter Wave, MTN MoMo, SPI et les moyens locaux.'
---
...
2. Un URL **webhook** HTTPS pour le statut final en mode live, voir [Webhooks](/build/webhooks).
...
| --- | --- |
| [Checkout hébergé](/build/checkout) | Votre app crée la commande et redirige le client |
| [Liens de paiement](/build/payment-links) | Vous voulez une URL partageable pour factures, services ou ventes simples |
| [Demandes de paiement](/build/payment-requests) | Votre backend crée une demande pour un flux dans votre app |
| [API de charge directe](/build/direct-charges) | Vous avez besoin d'un contrôle bas niveau et comprenez l'UX prestataire |
...
- **Webhooks**: enregistrez votre endpoint et traitez `PAYMENT_SUCCEEDED` / `PAYMENT_FAILED`. Voir [Recevoir les webhooks](/build/advanced-guides/handling-webhooks).
