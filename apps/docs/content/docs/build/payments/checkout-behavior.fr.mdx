...
- Les sessions sont en général **limitées dans le temps** (la fenêtre d’expiration par défaut est documentée sous [Sessions de paiement](/build/checkout)).
...
- **Sessions API**: à la création d’une session avec `product_id`/`price_id`, passez `amount` comme sous-total produit ou omettez-le pour utiliser le prix suggéré × quantité. Voir [Sessions de paiement](/build/checkout) et [Produits](/build/products).
...
Les champs produit définissent le **premier prélèvement au tunnel** (voir [Produits](/build/products)) :
...
Voir [Abonnements](/build/subscriptions) pour le cycle de vie et les webhooks.
...
- Plusieurs codes (lorsque c’est pris en charge) s’appliquent **séquentiellement** sur un montant courant, voir [Codes de réduction](/build/discount-coupons) et [Exemples de logique des codes](/build/payments/coupon-logic-examples).
...
Contrôlez quels champs client apparaissent sur le tunnel hébergé et s’ils sont obligatoires. Définissez les valeurs par défaut dans le tableau de bord (**Paramètres → Checkout**), surchargez par [lien de paiement](/build/payment-links), ou passez les indicateurs sur chaque [session checkout](/build/checkout).

### Champs système

| Champ | Indicateur API | Défaut si omis | Quand `false` |
| --- | --- | --- | --- |
| Nom | `require_name` | `true` | Masqué sur le formulaire |
| E-mail | `require_email` | `true` | Masqué sur le formulaire |
| Téléphone | `require_phone` | `false` | Affiché en optionnel |
| Adresse de facturation | `require_billing_address` | `false` | Masquée sur le formulaire |

<Callout type="info">
  **Contact :** la visibilité de l’e-mail et du téléphone suit les indicateurs ci-dessus. Si les deux sont masqués (`require_email: false` et `require_phone: false`), aucun des deux champs n’est affiché. Le téléphone peut rester obligatoire pour les moyens mobile money lorsqu’il est visible. Si le nom est masqué, la fiche client enregistre tout de même un nom d’affichage dérivé du nom saisi, de l’e-mail ou du téléphone (dans cet ordre).
</Callout>
...
1. **Session checkout** (`require_email`, `require_phone`, `require_name`, `require_billing_address` sur `POST /checkout-sessions`)
...
4. **Défauts plateforme** (e-mail obligatoire, téléphone optionnel, adresse masquée)

Passez `customer_email`, `customer_phone` et les champs d’adresse sur la session pour **préremplir** le formulaire ; cela ne change pas les champs affichés.
...
- [Abonnements](/build/subscriptions)
