...
<Callout type="info">
  **Contact collection:** checkout always collects at least one reachable contact method. If email is hidden and phone is optional, phone becomes required. If both email and phone are visible, email is required and phone stays optional unless you set `require_phone: true`.
</Callout>
...
1. **Checkout session** (`require_email`, `require_phone`, `require_billing_address` on `POST /checkout-sessions`)
