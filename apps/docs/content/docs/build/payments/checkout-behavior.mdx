...
See [Subscriptions: Renewals and failed payments](/build/subscriptions#renewals-and-failed-payments) for the full model.
...
Control which customer fields appear on the hosted checkout form and whether they are required. Configure defaults in the dashboard under **Settings → Checkout**, override per [payment link](/build/payment-links), or set flags on each [checkout session](/build/checkout).

### System fields

| Field | API flag | Default when unset | When `false` |
| --- | --- | --- | --- |
| Name | `require_name` | `true` | Hidden on the form |
| Email | `require_email` | `true` | Hidden on the form |
| Phone | `require_phone` | `false` | Shown as optional |
| Billing address | `require_billing_address` | `false` | Hidden on the form |

<Callout type="info">
  **Contact collection:** email and phone visibility follow the flags above. When both are hidden (`require_email: false` and `require_phone: false`), neither field is shown. Phone may still be required for mobile-money payment methods when it is visible. When the name field is hidden, the customer record still stores a display name derived from the typed name, email, or phone (in that order).
</Callout>
...
1. **Checkout session** (`require_email`, `require_phone`, `require_name`, `require_billing_address` on `POST /checkout-sessions`)
2. **Payment link** (same flags on the link that created the session)
3. **Organization checkout settings** (dashboard defaults)
4. **Platform defaults** (email required, phone optional, billing address hidden)

Pass `customer_email`, `customer_phone`, and address fields on the session to **pre-fill** the form; they do not change which fields are shown.
