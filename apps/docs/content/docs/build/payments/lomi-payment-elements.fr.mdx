...
<Tabs items={['Web (JS)', 'React Native', 'Tunnel intégré']}>
  ...
  <Tab value="Tunnel intégré">
    Guide complet : [Tunnel de paiement intégré](/build/embed-widget).
...
Voir [Documentation Webhooks](/build/webhooks).
...
Pour les numéros de carte de test, le mobile money (Wave, MTN MoMo), la QA sur checkout hébergé et les soldes de test, voir **[Paiements en bac à sable](/start/sandbox-payments)**.
