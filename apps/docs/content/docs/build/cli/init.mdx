...
It may also offer to run [`lomi install-rules`](/build/cli/install-rules) if you haven't installed agent rules yet, and optionally install a [lomi. UI](/build/lomi-ui) checkout component via [`lomi ui`](/build/cli/ui).

## Files created

| File | Purpose |
| --- | --- |
| `.env` | `LOMI_API_KEY`, `LOMI_WEBHOOK_SECRET` placeholder, `LOMI_API_URL` |
...
- [TypeScript SDK](/build/sdks/typescript)
