...
<Callout type="info" emoji="💡" title="Not the same as LOMI_API_KEY">
  The CLI token from `lomi login` authenticates **CLI commands**. Your application's secret API key (`LOMI_API_KEY` in `.env`) is configured separately via [`lomi init`](/build/cli/init).
</Callout>
...
- [Command reference](/build/cli/commands)
