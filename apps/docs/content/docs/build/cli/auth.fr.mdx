...
<Callout type="info" emoji="💡" title="Différent de LOMI_API_KEY">
  Le jeton CLI issu de `lomi login` authentifie les **commandes CLI**. La clé API secrète de votre application (`LOMI_API_KEY` dans `.env`) se configure séparément via [`lomi init`](/build/cli/init).
</Callout>
