...
_(Other event payloads are documented under their API resources, e.g. Transactions, Subscriptions.)_
...
Roughly speaking: **`4xx` client errors are usually terminal** for that automated sequence (misconfiguration should be corrected on your side), while **transient failures** (`5xx`, timeouts, certain network faults) may be retried up to a bounded number of attempts with exponential backoff. See **[Webhook reliability: Timeouts and retries](/build/advanced-guides/webhook-reliability#when-lomi-retries-and-when-it-does-not)** for the approximate **~4 second** outbound HTTP timer, up to **four** attempts on some paths, up to **five** when delivery is queued, and the meaning of **`lomi_environment`**.
