...
<Tabs items={['Web (JS)', 'React Native', 'Embedded Checkout']}>
  <Tab value="Web (JS)">```bash npm install @lomi./sdk ```</Tab>
...
Detailed endpoint docs: [Create card charge](/api/charge/ChargesController_createCardCharge)
...
<Tabs items={['Web (JS)', 'React Native', 'Embedded Checkout']}>
  ...
  <Tab value="Embedded Checkout">
    See the full guide: [Embed checkout widget](/build/accept/embed-widget).
...
See [Webhooks Documentation](/build/reliability) for setup details.
...
For sandbox test card numbers, mobile money (Wave, MTN), hosted checkout QA, and test balances, see **[Sandbox payments](/start/sandbox-payments)**.
