...
`Idempotency-Key` is **required** on money-moving writes: `POST /checkout-sessions`, `POST /payment-requests`, `POST /refunds`, `POST /payouts`, `POST /settlements/instant`, and `POST /charge/*`. Missing keys return `400` `idempotency_key_required`.
...
- [Error handling](/build/reliability/error-handling)
