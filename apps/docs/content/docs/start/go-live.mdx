...
2. When ready for real payments, the agent calls `POST /provisioning/merchants/{id}/live-activation/request` and shares the **merchant approval link** with you.
3. Open `/connect/go-live` on the dashboard, review the request, and approve go-live yourself.
