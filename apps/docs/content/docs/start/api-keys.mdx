...
| `Idempotency-Key` | On money-moving writes | Required on `POST /checkout-sessions`, `POST /payment-requests`, `POST /refunds`, `POST /payouts`, and `POST /charge/*`. See [Idempotency keys](/build/reliability/idempotency-keys). |
