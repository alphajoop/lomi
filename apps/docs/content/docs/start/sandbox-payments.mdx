...
For how live balances work after completion, see **[Balance and settlement](/build/balance-and-settlement)**.
...
For **[lomi Payment Elements](/build/payments/lomi-payment-elements)** or the Payment Intents API: create a payment intent with **`lomi_sk_test_…`**, mount the card UI with **`lomi_pk_test_…`**, and confirm using the returned **`client_secret`**. Use the test numbers above in the card field.
...
**Direct charge scenarios (`X-Scenario-Key`, test key only):** On `POST /charge/mtn` and `POST /charge/wave`, send `X-Scenario-Key: pending` to keep the charge in `PENDING` without auto-completing, or `failed` to simulate a `400` error. Omit the header for default test behavior.

For live MTN integration and sandbox MSISDNs, see the [MTN MoMo developer portal](https://momodeveloper.mtn.com/).
...
Beneficiary **Wave** payouts require `recipient.name` and `recipient.phone` on a **live** key. The phone number does not need to match a registered `payout_method_id`. See **[Payouts](/build/payouts)**.
...
Use the returned `client_secret` with **`lomi_pk_test_…`** on your client. See **[Direct charges](/build/payments/charges)** and **[lomi Payment Elements](/build/payments/lomi-payment-elements)**.

</Tab>

<Tab value="Payment link">

Create payment links in **Test** mode in the dashboard, or via the API with your test key so `environment` is `test`. See **[Payment links](/build/payment-links)**.
...
1. **Create a recurring product** with `trial_enabled: true` or `first_payment_type: non_initial` (see [Products](/build/products)).
...
See **[Webhooks](/build/webhooks)** for subscription management and verification details. For automated webhook testing patterns, see the **[Testing guide](/build/advanced-guides/testing)**.
...
- **[Authentication](/start/api-keys)** — keys, environments, security
- **[API integration](/start/first-payment)** — first request and API index
- **[Checkout sessions](/build/checkout)** — hosted checkout API
- **[Direct charges](/build/payments/charges)** — Wave, MTN, and embedded card (`POST /charge/card`)
- **[Payment links](/build/payment-links)** — reusable pay links
- **[lomi Payment Elements](/build/payments/lomi-payment-elements)** — client-side card UI
- **[Webhooks](/build/webhooks)** — event subscriptions
- **[Balance and settlement](/build/balance-and-settlement)** — live balance rules
- **[Testing guide](/build/advanced-guides/testing)** — automation, CI, webhook listeners

## When am I ready for live mode?

Move to live keys only when your integration is verified end to end. See the [go-live checklist](/start/go-live).
