...
Choisissez la surface qui correspond à votre façon de travailler. Chaque onglet est un point de départ. Les guides complets restent sur leurs pages : [API](/api), [Démarrer avec le SDK](/start/sdk-quickstart), [Démarrer avec le CLI](/start/cli-quickstart) et [MCP](/build/mcp).
...
Ce choix de produit est indépendant de l’API, du SDK, du CLI ou du MCP. Voir [Comment utiliser lomi.](#how-you-call-lomi).
