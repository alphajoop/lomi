...
Pour le fonctionnement des soldes live après complétion, voir **[Soldes et règlement](/build/balance-and-settlement)**.
...
Avec **[lomi Payment Elements](/build/payments/lomi-payment-elements)** ou l’API Payment Intents : créez un payment intent avec **`lomi_sk_test_…`**, montez l’UI carte avec **`lomi_pk_test_…`**, puis confirmez avec le **`client_secret`** retourné. Utilisez les numéros de test ci-dessus dans le champ carte.
...
Pour les numéros autorisés en bac à sable MTN, voir le [portail développeur MTN MoMo](https://momodeveloper.mtn.com/).
...
Les virements **Wave** vers un tiers exigent `recipient.name` et `recipient.phone` avec une **clé live**. Le numéro n’a pas besoin de correspondre à un `payout_method_id`. Voir **[Virements](/build/payouts)**.
...
Utilisez le `client_secret` avec **`lomi_pk_test_…`** côté client. Voir **[Encaissements directs](/build/payments/charges)** et **[lomi Payment Elements](/build/payments/lomi-payment-elements)**.

</Tab>

<Tab value="Lien de paiement">

Créez des liens en mode **Test** dans le tableau de bord, ou via l’API avec votre clé de test pour `environment: test`. Voir **[Liens de paiement](/build/payment-links)**.
...
1. **Créez un produit récurrent** avec `trial_enabled: true` ou `first_payment_type: non_initial` (voir [Produits](/build/products)).
...
Voir **[Webhooks](/build/webhooks)**. Pour l’automatisation, voir le **[Guide de tests](/build/advanced-guides/testing)**.
...
- **[Authentification](/start/api-keys)** — clés, environnements, sécurité
- **[Intégration API](/start/first-payment)** — première requête et index API
- **[Sessions checkout](/build/checkout)** — API checkout hébergé
- **[Encaissements directs](/build/payments/charges)** — Wave, MTN et carte (`POST /charge/card`)
- **[Liens de paiement](/build/payment-links)** — liens réutilisables
- **[lomi Payment Elements](/build/payments/lomi-payment-elements)** — UI carte côté client
- **[Webhooks](/build/webhooks)** — abonnements aux événements
- **[Soldes et règlement](/build/balance-and-settlement)** — règles des soldes live
- **[Guide de tests](/build/advanced-guides/testing)** — automatisation, CI, écouteurs webhook

## Passer en production

Passez aux clés live uniquement quand votre intégration est validée de bout en bout. Voir la [checklist de mise en production](/start/go-live).
