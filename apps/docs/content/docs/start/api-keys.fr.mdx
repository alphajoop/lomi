...
| `Idempotency-Key` | Sur les écritures qui bougent de l’argent | Obligatoire sur `POST /checkout-sessions`, `POST /payment-requests`, `POST /refunds`, `POST /payouts` et `POST /charge/*`. Voir [Clés d’idempotence](/build/reliability/idempotency-keys). |
