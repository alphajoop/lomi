...
| Docs scan / graph | `lomi docs scan`, `lomi docs graph` (delegates to `apps/doctool`) |
...
Documentation tooling CLI (CORE-38). **Standalone repo:** [github.com/lomiafrica/doctool](https://github.com/lomiafrica/doctool) (not a git submodule).

Clone into the monorepo for local dev:
...
Maintainer docs: [`apps/doctool/CONTRIBUTING.md`](https://github.com/lomiafrica/doctool/blob/main/CONTRIBUTING.md)

| Task | Command / location |
| --- | --- |
| Local dev | `cd apps/doctool && cargo build && cargo test` |
...
| Agent OpenAPI | `cd apps/api && pnpm run openapi:export:agent` → `apps/docs/agent-openapi.json` |
